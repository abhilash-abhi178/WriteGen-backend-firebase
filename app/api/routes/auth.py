...
async def update_user(
    ...
    if update.privacy:
        current_user["privacy"] = update.privacy.dict()
...
async def delete_account(current_user: dict = Depends(get_current_user)):
    """Delete user account (scheduled 30-day deletion)."""
    user_id = current_user["user_id"]
    
    # Schedule deletion
    current_user["deletion_scheduled"] = datetime.utcnow()
    current_user["deletion_date"] = datetime.utcnow() + timedelta(days=30)
    users_db[user_id] = current_user
    
    logger.info(f"User {user_id} scheduled for deletion in 30 days")
    
    return {
        "message": "Account scheduled for deletion",
        "deletion_date": current_user["deletion_date"]
    }
